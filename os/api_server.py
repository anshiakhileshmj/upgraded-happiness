...
# Configure CORS for React frontend - Allow all origins for development
app.add_middleware(
    CORSMiddleware,
...
# Direct automation endpoint that processes speech input
class DirectAutomationRequest(BaseModel):
    objective: str

class DirectAutomationResponse(BaseModel):
    success: bool
    message: str
...
@app.post("/direct-automate", response_model=DirectAutomationResponse)
async def direct_automate(request: DirectAutomationRequest):
    """
    Direct automation endpoint that mimics the CLI experience.
    Takes an objective, generates actions, and executes them automatically.
    """
    try:
        logger.info(f"Direct automation request: {request.objective}")
        
        # Validate config
        config.validation()
        
        # Create system message for the objective
        system_prompt = f"You are an AI assistant that automates computer tasks. The user wants: {request.objective}"
        system_message = {"role": "system", "content": system_prompt}
        messages = [system_message]
        
        # Generate actions using the existing get_next_action function
        operations, session_id = await get_next_action("gemini-1.5-flash", messages, request.objective, None)
        
        if not operations:
            return DirectAutomationResponse(
                success=False,
                message="No automation actions could be generated for this request",
                error="Failed to generate actions"
            )
        
        logger.info(f"Generated {len(operations)} operations for: {request.objective}")
        
        # Execute the operations using the existing operate function
        executed_count = 0
        try:
            # The operate function returns True when it should stop (task complete)
            stop = operate(operations, "gemini-1.5-flash")
            executed_count = len(operations)
            
            if stop:
                return DirectAutomationResponse(
                    success=True,
                    message=f"Successfully completed: {request.objective}",
                    actions_executed=executed_count
                )
            else:
                return DirectAutomationResponse(
                    success=True,
                    message=f"Executed {executed_count} actions for: {request.objective}",
                    actions_executed=executed_count
                )
        ...
        logger.error(f"Error in direct automation: {str(e)}")
        return DirectAutomationResponse(
            success=False,

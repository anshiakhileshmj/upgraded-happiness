...
export class ActionRouter {
  async processUserInput(
    ...
    isAutomateEnabled: boolean = false
...
- "automate_action": any request to automate a task on the computer (open, launch, start, type, click, close, etc.)
...
Respond ONLY with valid JSON.`;
  ...
  private async handleAutomateAction(params?: Record<string, any>): Promise<ActionResult> {
    try {
      ...
      console.log('Processing automation objective:', objective);

      // Use the new direct automation method that mimics CLI behavior
      const result = await automateService.directAutomate(objective);
